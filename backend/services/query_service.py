...
async def answer_query(user_id: str, query: str, language: str = "en") -> str:
    ...
    if supabase is not None:
        ...
        logger.warning("Supabase client is None, using in-memory search only")
        chunks = search_chunks(query_embedding, TOP_K)
...
{language_instruction}{memory_block}"""
    ...
    if anthropic_client is None:
        logger.error("Anthropic client is None — ANTHROPIC_API_KEY is likely missing")
        return "I apologize, but my AI service is not configured. Please contact the administrator."
